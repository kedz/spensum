...
def make_dataset(tsne_data, rank_data, batch_size, gpu):
    
    lengths = torch.LongTensor([len(dp["text"]) for dp in tsne_data])
    ...
    for i, (tsne_dp, rank_dp) in enumerate(zip(tsne_data, rank_data)):
        
        assert tsne_dp["id"] == rank_dp["id"]
        ids.append(tsne_dp["id"])
        texts.append(tsne_dp["text"])
        
        for j in range(len(rank_dp["ranks"])):      
            sequence[i,j,0] = tsne_dp["salience"][j]      
            sequence[i,j,1] = tsne_dp["word_count"][j]
            sequence[i,j,2] = tsne_dp["tsne"][j][0]
            sequence[i,j,3] = tsne_dp["tsne"][j][1]
        #print(sequence[i,:len(rank_dp["ranks"])])
        ex_ranks = [(i, rank) for i, rank in enumerate(rank_dp["ranks"]) if rank > 0]
...
def main():
    ...
    training_data = make_dataset(
        training_tsne_data, training_ranks_data, args.batch_size, args.gpu)
    ...
    validation_data = make_dataset(
        validation_tsne_data, validation_ranks_data, args.batch_size, args.gpu)
