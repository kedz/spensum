...
#        "--save-predictor", default=None, required=False, type=str)
    ...
    model = spensum.model.RNNCRFExtractor(
...
#    print("# non-salient = {}".format(non_salient_count))
    ...
    crit = ntp.crf.criterion.CrossEntropy()
    crit.add_reporter(
        ntp.criterion.SequenceMultiClassFMeasureReporter(
            2, labels=["skip", "extract"]))


        #mode="prob", weight=weight, mask_value=-1)
    #crit.add_reporter(
    #    ntp.criterion.SequenceMultiClassFMeasureReporter(mode="prob"))
