...
#        "--save-predictor", default=None, required=False, type=str)
    ...
    input_reader = spensum.dataio.init_duc_sds_input_reader(
        args.embedding_size)
    rank_reader = spensum.dataio.init_duc_sds_pairwise_rank_reader()

    train_dataset = spensum.dataio.read_input_rank_dataset(
        args.train_inputs, args.train_ranks, 
        input_reader, rank_reader,
        batch_size=args.batch_size, gpu=args.gpu)

    valid_dataset = spensum.dataio.read_input_rank_dataset(
        args.valid_inputs, args.valid_ranks, 
        input_reader, rank_reader,
        batch_size=args.batch_size, gpu=args.gpu)
...
#    print("# non-salient = {}".format(non_salient_count))
    ...
    crit = spensum.criterion.RankLoss(mask_value=-1)
#    crit.add_reporter(
