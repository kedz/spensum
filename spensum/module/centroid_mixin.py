...
class CentroidMixIn(object):
    ...
    def zero_identity_mask(self, batch_size, input_size, fill_value=1.0):
        ''' 
        ...
        '''
        mask = torch.FloatTensor(input_size, input_size).fill_(fill_value) - \
            torch.diag(torch.FloatTensor(input_size).fill_(fill_value))
    ...
    def centroid(self, inputs, mask, remove_identity=False):
        ...
        if remove_identity:
            zi_mask = self.zero_identity_mask(batch_size, input_size)
